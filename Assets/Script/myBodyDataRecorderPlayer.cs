...
namespace com.rfilkov.components
{
    ...
    /// </summary>
    public class myBodyDataRecorderPlayer : MonoBehaviour
    {
        ...
        private static myBodyDataRecorderPlayer instance = null;
...
//        private StreamReader fileReader = null;
        ...
        public static myBodyDataRecorderPlayer Instance
        {
            get
            {
                return instance;
        ...
        public bool StartRecording()
        {
            ...
            if (isRecording)
            {
                ...
                if (filePath.Length > 0 && File.Exists("Assets/Resources/" + filePath))
                {
                    Debug.Log("Delete file " + filePath);
                    File.Delete("Assets/Resources/" + filePath);
                }
...
//                csv = Resources.Load(filePath) as TextAsset;
                csv = Resources.Load(filePath.Split('.')[0]) as TextAsset;
                if(csv==null){
                    Debug.LogError("Resources.Load: File not found " + filePath);
//                    Debug.Log("csv=" + csv);
                    infoText.text = "Resources.Load: File not found " + filePath;
                   isPlaying = false;
                }
            }
...
//                fileReader = new StreamReader(filePath);
                sReader = new StringReader(csv.text);
#endif
                ReadLineFromFile();
        ...
        void Update()
        {
            if (isRecording)
            {
                ...
                if (kinectManager && kinectManager.IsInitialized() && liRelTime != kinectManager.GetBodyFrameTimestamp())
                {
                    ...
                    {
#if !UNITY_WSA
                        using (StreamWriter writer = File.AppendText("Assets/Resources/" + filePath))
                        {
        ...
        {
//            if (fileReader == null)
//            Debug.Log("in ReadLineFromFile");
            if(sReader.Peek()<0)
                return false;

            // read a line
//            sPlayLine = fileReader.ReadLine();
            sPlayLine = sReader.ReadLine();
//            Debug.Log("sPlayLine=" + sPlayLine);
            if (sPlayLine == null)
                return false;
