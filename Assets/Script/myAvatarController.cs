...
namespace com.rfilkov.components
{
    ...
    public class myAvatarController : MonoBehaviour
    {
        ...
        // Added by T.Narumi
        [Tooltip("Maximum number of avaters which is used by E-key to change avater")]
        public int MaxplayerIndex = 1;
        [Tooltip("Translate avatar around -10 to hide even if Kinect ditects a user if Q key is pressed")]
        public bool disableAvatarByQkey = false;
        [Tooltip("If true choki is used instead of goo")]
        public bool choki_or_goo = false;
        ...
        // Added by T.Narumi
        // private instance of the mySimpleGestureListener
        protected mySimpleGestureListener mysimpleGestureListener;
        ...
        // Added by T.Narumi
        private int keyq = 0;
        private int playeroffset = 0, playeroffset_old = 0;
        ...
        public void Update()
        {
//            Debug.Log("in Update: choki_or_goo="+choki_or_goo);


            // Added by T.Narumi
            if(mysimpleGestureListener == null)
            {
                mysimpleGestureListener = mySimpleGestureListener.Instance;
            }
            playeroffset = mysimpleGestureListener.GetPlayeroffset();
            if (disableAvatarByQkey){
                if(Input.GetKeyDown(KeyCode.Q)){
                    keyq = 1 - keyq;
                } else if(Input.GetKeyDown(KeyCode.E)){
                    playerIndex ++;
                    if(playerIndex>=MaxplayerIndex) playerIndex = 0;
                    //Debug.Log("E-key: playerIndex=" + playerIndex);
                }
                if(playeroffset_old != playeroffset){
                    if(playeroffset_old < playeroffset) playerIndex ++;
                    else                                playerIndex --;
                    //playerIndex += playeroffset;
                    while(playerIndex>=MaxplayerIndex) playerIndex -= MaxplayerIndex;
                    while(playerIndex<0)               playerIndex += MaxplayerIndex;
                    playeroffset_old = playeroffset;
                    Debug.Log("Offset: playerIndex=" + playerIndex + " playeroffset=" + playeroffset);
                }    
            }
            // Change the face during run
            runflag = mysimpleGestureListener.GetRunflag();
            if(runflag_bak==0 && runflag==1){
//                facecontroller.ChangeFace(BlendShapePreset.Joy, false);
                facecontroller.ChangeFace(BlendShapePreset.Joy, false); 
//                facecontroller.ChangeFace(BlendShapePreset.Angry, false); 
                Debug.Log("Runflag becomes 1");
            } else if(runflag_bak==1 && runflag==2){
                facecontroller.ChangeFace(BlendShapePreset.Fun, false);
//                facecontroller.ChangeFace(BlendShapePreset.Sorrow, true);
                Debug.Log("Runflag becomes 2");
            } else if(runflag_bak!=0 && runflag==0){
                facecontroller.ChangeFace(BlendShapePreset.Neutral, false);
                Debug.Log("Runflag becomes 0");
            }
            runflag_bak = runflag;

            /*
            // get the state of hands
            ...
            if (playerUserID != 0){
                ...
                Debug.Log("playerUserID=" + playerUserID + " handState=" + handState + 
                        " LhandState=" + kinectManager.GetLeftHandState(playerUserID));
//                Debug.Log("userId=" + userId + " handState=" + kinectManager.GetLeftHandState(userId));
            }
            */
...
//            Debug.Log("user id = " + userId);
            ...
            // Added by T.Narumi
            if(userId == 0 || keyq == 1) this.gameObject.transform.position = new Vector3(0.0f, 0.0f, -10.0f);
//            else            this.gameObject.transform.position = new Vector3(0.0f, 0.0f,  10.0f);
		...
		//                else if (boneIndex == 21 || boneIndex == 23) // Modified by T.Narumi
        ...
        protected void TransformSpecialBoneFingers(ulong userId, int joint, int boneIndex, bool flip)
        {
			...
			//                        float angleRot = angleSign * 60f;
                        float angleRot = angleSign * 75f; // Modified by T.Narumi
	    ...
	    float angle2;
	      ...
	      //	      if (boneIndex == 22 || boneIndex == 24)  // Modified by T.Narumi
	      //		    continue;
	      
	      // Added by T.Narumi
//	        Debug.Log("choki_or_goo="+choki_or_goo+" boneIndex="+boneIndex+" i="+i);
		angle2 = angle;
	        Vector3 ax = fingerBoneLocalAxes[bone];
                if (choki_or_goo == true){ // Choki
		  if( boneIndex == 22 || boneIndex == 24 ){
		    if(i==0){
		      angle2=-80.0f;//-120.0f;
		      //		      ax= new Vector3(0.0f, 1.0f, 0.0f);
		    } else {
		      angle2=-120;
		    }
		  }
		  if(i < 6 && (boneIndex == 21 || boneIndex == 23)){
		    if(i!=0) continue; // Added by T.Narumi
		    ax=new Vector3(0.0f,1.0f,0.0f);
		    if(boneIndex==21) angle2=30.0f;
		    else              angle2=-30.0f;
		  }
                } else {                   // Goo
		  if((boneIndex == 22 || boneIndex == 24)) continue; // Added by T.Narumi
		  //                    continue; // Added by T.Narumi
                }
                ...
                {
		  //		    Debug.Log("bone="+bone+" angle2="+angle2+" fingerBoneLocalAxes="+ax);
		    //                    Quaternion qRotFinger = Quaternion.AngleAxis(angle, fingerBoneLocalAxes[bone]);
                    Quaternion qRotFinger = Quaternion.AngleAxis(angle2, ax); // Modified by T.Narumi
